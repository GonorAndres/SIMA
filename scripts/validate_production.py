...
check(
    "A2",
    health["engine_modules"] == ENGINE_MODULE_COUNT,
...
check(
    "B1",
    0.70 <= lc["explained_variance"] <= 0.90,
    f"explained_var={lc['explained_variance']:.4f} (expect 0.70-0.90)",
)
check("B2", -1.3 <= lc["drift"] <= -0.8, f"drift={lc['drift']:.4f} (expect -1.3 to -0.8)")
check("B3", lc["sigma"] > 0, f"sigma={lc['sigma']:.4f}")
check(
    "B4",
    lc["validations"]["bx_sums_to_one"] and lc["validations"]["kt_sums_to_zero"],
...
check(
    "C1",
    5_000 <= wl40["annual_premium"] <= 40_000,
...
check(
    "C2",
    term40["annual_premium"] < wl40["annual_premium"],
...
check(
    "C3",
    endow40["annual_premium"] > wl40["annual_premium"],
...
check(
    "C4",
    ages_prems[0] < ages_prems[1] < ages_prems[2],
...
check("C5", male30 > female30, f"male(${male30:,.0f}) > female(${female30:,.0f})")

# Cross-country
cc = post(
    "/pricing/cross-country",
    {
        "product_type": "whole_life",
        "age": 40,
        "sum_assured": 1_000_000,
...
check(
    "C6", mx_p > usa_p > spain_p, f"MX(${mx_p:,.0f}) > USA(${usa_p:,.0f}) > Spain(${spain_p:,.0f})"
)
...
check(
    "E1",
    shock["premiums"][2] > shock["premiums"][1] > shock["premiums"][0],
...
check(
    "E2",
    covid["full_period"]["drift"] > covid["pre_covid"]["drift"],
