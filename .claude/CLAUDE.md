...
| ~~H3~~ ✅ FIXED (2026-08-03) | Portfolios are now keyed by an opaque session cookie minted in `backend/api/dependencies.py`, with 1h TTL expiry and a 500-session LRU cap so anonymous callers cannot grow the map. Callers that send no cookie share `DEFAULT_SESSION`, preserving the old behaviour for the test suite and curl. Regression test: `test_portfolio_api.py::test_portfolio_is_per_session`. | `backend/api/services/scr_service.py`, `backend/api/dependencies.py` | **Remaining limit, documented at the code:** the map is per-process and Cloud Run does not pin sessions to an instance, so a caller can occasionally land on an instance that has never seen their session and get the sample portfolio back. Fixing that properly means moving the portfolio out of the process. |

#### Medium Priority
| ID | Issue | File | Fix |
|----|-------|------|-----|
| ~~M1~~ ❌ ALREADY FIXED — CLOSED (2026-08-03) | The scattered per-router handlers are gone. Every route is wrapped by the `safe_route` decorator, which funnels through `map_exception_to_response()`; that logs the full trace with `logger.exception()` and returns a sanitized 500 so stack traces never reach the client. | `backend/api/exception_handlers.py` | Nothing to do. Verified: `grep -rn 'except Exception' backend/api/routers/` returns nothing. |
...
| ~~M4~~ ❌ PREMISE WRONG — CLOSED (2026-08-03) | No crash: the `pure_endowment` branch exists in both `validate_zero_reserve` and `reserve_trajectory`, and 0V comes back at ~6e-11. The real gap was one layer up — the product was absent from the frontend, which hid two genuine bugs (a 422 from `SensitivityRequest`, whose product Literal was narrower than `PremiumRequest`'s, and a missing label that rendered the raw `PURE_ENDOWMENT`). | `frontend/src/components/forms/PremiumForm.tsx`, `backend/api/schemas/pricing.py` | Fixed 2026-08-03 together with L5. Reserve + sensitivity tests added. |
...
| ~~M6~~ ✅ FIXED (2026-08-03) | Slider max raised 70 → 90. Verified the API prices the new ceiling (age 90 whole life = $177,694.62). | `frontend/src/components/forms/PremiumForm.tsx` | Done. |
| ~~M7~~ ✅ FIXED (2026-08-03) | `projection_year` now defaults to `None` on `/mortality/projection` and `/mortality/validation`, and the service derives the end of the projected window at runtime. Both responses echo `projection_year` back so the UI labels the window it actually plots. This also fixed a visible defect: the k_t chart was titled "2020-2040" while plotting a 30-year horizon to 2049. | `backend/api/routers/mortality.py`, `backend/api/services/mortality_service.py` | Note: `horizon` truncates the plotted k_t series but must NOT narrow which years a life table can be built for — conflating the two broke `test_projection`. |
...
| ID | Issue | File |
|----|-------|------|
| ~~L1~~ ❌ MEASURED, NOT A BOTTLENECK — WON'T FIX (2026-08-03) | 0.069 ms per rebuild over 101 ages, and it is called 3× per SCR run: **0.2 ms total**. Caching would add invalidation risk (`LifeTable` is unhashable, so any cache keys on identity) to save a fifth of a millisecond. Re-open only with a profile showing it matters. | `backend/engine/a12_scr.py:183` |
| ~~L2~~ ✅ FIXED (2026-08-03) | `validate()` mixed correctness checks with scenario-dependent descriptions. Split: `sigma_positive` / `no_nan_in_central` must hold; `drift_indicates_improvement` / `central_continues_downward` merely describe, since a deterioration scenario legitimately makes both False. Old keys retained as aliases. | `backend/engine/a09_projection.py` |
| ~~L3~~ ⚠️ PARTLY WRONG — RESOLVED (2026-08-03) | The hooks are not identical: each fetches a different `table_type`, separately so one table failing cannot take down the section, and prefetching makes tab switches instant. That stays. What *was* triplicated — two parallel ternary chains picking the active hook and the retry target — is now a single `validations` record. | `frontend/src/pages/Mortalidad.tsx` |
| ~~L4~~ ✅ FIXED (2026-08-03, by the M2 rewrite) | No hardcoded k_t remains; `cross_country_data()` and `covid_comparison()` both read the live pipelines, so they cannot disagree. | `backend/api/services/sensitivity_service.py` |
| ~~L5~~ ✅ FIXED (2026-08-03) | Dropdown option, term field, translated label and hint in both languages, and the premium formula P = SA · D_{x+n} / (N_x − N_{x+n}). See M4 for the two bugs this exposed. | `frontend/src/components/forms/PremiumForm.tsx`, `frontend/src/pages/Tarificacion.tsx` |
| ~~L6~~ ❌ ALREADY FIXED — CLOSED (2026-08-03) | No `sleep` remains in `deploy.yml`. | `.github/workflows/deploy.yml` |
| ~~L7~~ ✅ FIXED (2026-08-03) | Now `CMD ["sh", "-c", "exec uvicorn ... --port ${PORT:-8080}"]`. `${PORT}` still needs shell expansion, but `exec` replaces the shell so uvicorn becomes PID 1 and receives SIGTERM. Verified empirically: under the old shell form the workload **survived** SIGTERM (orphaned until SIGKILL); under the new form it terminates. | `Dockerfile` |
