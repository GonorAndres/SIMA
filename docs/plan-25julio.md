...
- **Phase 3 — Data pipeline robustness:** ✅ implementation and tests complete locally, pending commit — new `backend/engine/data_validation.py` with schema validators and `DataQualityReport`; hardened `a06_mortality_data.py` (versioned HMD/INEGI schema, structured `DataQualityError`, optional missing-value imputation, open-age 110+ handling unchanged); hardened `a07_graduation.py` (lambda >= 0, diff_order >= 1, configurable residual threshold, monotonicity warning, ill-conditioned system warning); hardened `a08_lee_carter.py` (configurable explained-variance threshold; adaptive brentq bracket already present); hardened `a09_projection.py` (positive horizon/n_simulations bound, np.searchsorted bounds, age range non-empty validation); hardened `a10_validation.py` (NaN-safe ratio summary, weighted RMSE, bias, configurable summary age range). Added 19 focused regression tests for malformed schemas, data-quality reports, graduation pathologies, projection bounds, and validation metrics. Phase 3 subset: 102 passed; ruff and strict mypy pass.
- **Verification note (2026-07-25):** The full 353-test suite could not be completed in the exe.dev sandbox because FastAPI `TestClient` hangs while entering application lifespan; `load_all()` itself completes normally and all 102 Phase 3 tests pass. Recheck the API suite in CI or a non-sandboxed environment before committing.
- **Next:** Resolve or externally verify the API `TestClient` lifespan hang, then commit Phase 3. Afterward begin Phase 6 documentation.
