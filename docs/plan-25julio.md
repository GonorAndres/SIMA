...
- **Phase 3 — Data pipeline robustness:** 🔄 in progress — new `backend/engine/data_validation.py` with schema validators and `DataQualityReport`; hardened `a06_mortality_data.py` (versioned HMD/INEGI schema, structured `DataQualityError`, optional missing-value imputation, open-age 110+ handling unchanged); hardened `a07_graduation.py` (lambda >= 0, diff_order >= 1, configurable residual threshold, monotonicity warning, ill-conditioned system warning); hardened `a08_lee_carter.py` (configurable explained-variance threshold; adaptive brentq bracket already present); hardened `a09_projection.py` (positive horizon/n_simulations bound, np.searchsorted bounds, age range non-empty validation); hardened `a10_validation.py` (NaN-safe ratio summary, weighted RMSE, bias, configurable summary age range). Tests still being added; Phase 3 will be finalized in a follow-up commit.
- **Next:** Complete Phase 3 tests, then commit.
