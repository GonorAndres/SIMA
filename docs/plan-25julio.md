...
- **Phase 3 — Data pipeline robustness:** ✅ done (`b9dd783`, tests finalized in `09ae998`) — new `backend/engine/data_validation.py` with schema validators and `DataQualityReport`; hardened `a06_mortality_data.py` (versioned HMD/INEGI schema, structured `DataQualityError`, optional missing-value imputation, open-age 110+ handling unchanged); hardened `a07_graduation.py` (lambda >= 0, diff_order >= 1, configurable residual threshold, monotonicity warning, ill-conditioned system warning); hardened `a08_lee_carter.py` (configurable explained-variance threshold; adaptive brentq bracket already present); hardened `a09_projection.py` (positive horizon/n_simulations bound, np.searchsorted bounds, age range non-empty validation); hardened `a10_validation.py` (NaN-safe ratio summary, weighted RMSE, bias, configurable summary age range). Added 19 focused regression tests for malformed schemas, data-quality reports, graduation pathologies, projection bounds, and validation metrics. Phase 3 subset: 102 passed; ruff and strict mypy pass.
- **Verification note (2026-07-26):** The full 353-test suite cannot execute in the exe.dev sandbox because even a minimal empty FastAPI application hangs in `TestClient.__enter__()` while AnyIO waits on its blocking portal; this reproduces without SIMA imports or lifespan logic. `load_all()` itself completes normally and all 102 Phase 3 tests pass. Treat this as an environment/dependency test-runner issue and verify the API suite in CI or a non-sandboxed environment.
- **Phase 6 — Documentation:** ✅ done — refreshed project spec and frontend quick-start; added `backend/VALIDATION.md`; documented SCR scope/calibration limitations; added HMD/INEGI/CONAPO download guides; reconciled active endpoint counts to 24 across current references. Frontend lint and production build pass; pytest collects 353 tests.
- **Actuarial programmer audit corrections:** ✅ complete locally — fixed contractual premiums under mortality/rate stress, standardized catastrophe risk on an in-force conditional basis, corrected endowment maturity BEL and undiscounted portfolio horizon, hardened empty/numeric/zero-rate boundaries, corrected inverse longevity calibration, exposed Lee–Carter root-fallback diagnostics, and repaired coherent missing-data imputation. Added 14 audit regressions; 305 non-HTTP tests pass and 367 total tests collect. Ruff, strict mypy, frontend lint, and frontend build pass.
- **Next:** Run the full HTTP suite in CI/non-sandboxed execution, then prepare the branch for PR review.
...
- Treat the portfolio consistently as an in-force inventory: apply attained-age
  conditional mortality without multiplying issue-to-date survival again.
